...
model = tf.keras.models.load_model('densenet169_model.keras')
...
val_test_datagen = tf.keras.preprocessing.image.ImageDataGenerator(preprocessing_function=tf.keras.applications.densenet.preprocess_input)
...
def plot_confusion_matrix(cm, class_names, save_path='confusion_matrix_densenet169.png'):
    ...
    plt.title('Confusion Matrix - DenseNet169', fontsize=18)
...
def plot_performance_metrics(metrics):
    ...
    plt.xlabel('Value')
    plt.title('Performance Metrics - DenseNet169')
    for index, value in enumerate(metrics_values):
        plt.text(value, index, f'{value:.4f}', va='center')
    plt.savefig('performance_metrics_densenet169.png')
    plt.close()
...
def plot_accuracy(history):
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig('model_accuracy_densenet169.png')
    plt.close()
...
def plot_loss(history):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('model_loss_densenet169.png')
    plt.close()
